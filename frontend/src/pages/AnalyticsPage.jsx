...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-brand-elevated border border-[var(--border-default)] rounded-lg px-3 py-2 text-xs shadow-xl">
      <p style={{ color: payload[0].payload.fill ?? '#bcbabb' }}>
        ...
        <section>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={priorityData} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(188,186,187,0.06)" vertical={false} />
              <XAxis
                dataKey="priority"
                tick={{ fill: 'rgba(188,186,187,0.4)', fontSize: 11, fontFamily: 'DM Sans' }}
                axisLine={false}
                tickLine={false}
              />
              <YAxis
              ...
              <Bar dataKey="count" radius={[4, 4, 0, 0]}>
                <Cell fill="#904e55" />
                <Cell fill="rgba(188,186,187,0.35)" />
                <Cell fill="#256a69" />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          ...
          <section>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={teamData} layout="vertical" margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                ...
                />
                <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(188,186,187,0.04)' }} />
                <Bar dataKey="open" name="Open" stackId="a" fill="#904e55" />
                <Bar dataKey="done" name="Done" stackId="a" fill="#256a69" radius={[0, 3, 3, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </section>
        )}
